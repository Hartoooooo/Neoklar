...
function ShaderBackground() {
  ...
  return (
    <div ref={canvasRef} className="bg-black absolute inset-0 -z-10 w-full h-full" style={{ left: 0, right: 0 }} aria-hidden>
      <Canvas
        camera={camera}
...
export default function Hero({
  ...
  // Stelle sicher, dass die Section die volle Breite hat
  useEffect(() => {
    if (sectionRef.current) {
      const section = sectionRef.current;
      section.style.width = '100vw';
      section.style.marginLeft = 'calc(-1 * (100vw - 100%) / 2)';
      section.style.marginRight = 'calc(-1 * (100vw - 100%) / 2)';
  ...
  useGSAP(
    () => {
      ...
      // Stelle sicher, dass die Section die volle Breite hat (auch nach GSAP-Animationen)
  ...
  return (
    <section ref={sectionRef} className="relative h-screen overflow-hidden" style={{ 
      position: 'relative',
      width: '100vw',
      marginLeft: 'calc(-1 * (100vw - 100%) / 2)',
      marginRight: 'calc(-1 * (100vw - 100%) / 2)',
